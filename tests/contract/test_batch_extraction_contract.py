...
def sample_mask_data_with_results():
    ...
    mask_data = MaskData(
        mask_img=img, space="MNI152NLin6Asym", resolution=2.0, metadata={"subject_id": "sub-001"}
    )
    ...
    voxel_map = VoxelMap(
        name="correlation_map", data=voxel_img, space="MNI152NLin6Asym", resolution=2.0
    )
    ...
    scalar_metric = ScalarMetric(name="lesion_volume", data=125.5)
...
def batch_results_with_parcel_data(sample_mask_data_with_results):
    ...
    for i in range(3):
        ...
        mask_data = mask_data.add_result("ParcelAggregation", {"parcel_means": parcel_data})
...
class TestExtractVoxelmapsContract:
    ...
    def test_extract_voxelmaps_returns_dict(self, sample_mask_data_with_results):
        ...
        voxelmaps = extract_voxelmaps(results, analysis="TestAnalysis", key="correlation_map")
...
class TestExtractParcelTableContract:
    ...
    def test_extract_parcel_table_returns_dataframe(self, batch_results_with_parcel_data):
        ...
        df = extract_parcel_table(
            batch_results_with_parcel_data, analysis="ParcelAggregation", key="parcel_means"
        )
...
class TestExtractScalarsContract:
    ...
    def test_extract_scalars_returns_series(self, sample_mask_data_with_results):
        ...
        results = [sample_mask_data_with_results]
        scalars = extract_scalars(results, analysis="TestAnalysis", key="lesion_volume")
...
class TestExtractionErrorHandling:
    ...
    def test_missing_result_returns_nan(self, batch_results_with_parcel_data):
        ...
        df = extract_parcel_table(
            batch_results_with_parcel_data, analysis="ParcelAggregation", key="nonexistent_key"
        )
