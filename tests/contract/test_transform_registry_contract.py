...
def test_load_transform_returns_path(tmp_path, monkeypatch):
    """load_transform downloads via pooch and returns a Path (network-free).

    The warps are fetched from OSF with pooch; mock pooch.retrieve so the
    loader wiring is exercised without touching the network.
    """
    ...
    def fake_retrieve(url, known_hash=None, fname=None, path=None, progressbar=False):
        captured["url"] = url
        captured["known_hash"] = known_hash
        out = Path(path) / fname
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_bytes(b"mock transform data")
        return str(out)
    ...
    # Loader passed the registered OSF url and pinned sha256 to pooch.
    assert captured["url"].startswith("https://osf.io/")
    assert captured["known_hash"].startswith("sha256:")


def test_load_transform_wraps_download_failure(monkeypatch):
    """A pooch download failure surfaces as a clear FileNotFoundError."""
    import pooch
    ...
    with pytest.raises(FileNotFoundError, match="Failed to download transform"):
