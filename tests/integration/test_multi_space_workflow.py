...
@pytest.mark.skip(
    reason="Automatic space transformation not yet fully implemented in analysis modules"
)
def test_cross_space_analysis_with_transformation(lesion_nlin6_2mm, atlas_nlin2009c_2mm, tmp_path):
    ...
    analyzer = LocalDamage()

    # This should succeed despite space mismatch
    result = analyzer.run(lesion_nlin6_2mm)

    # Verify results are present
    assert "LocalDamage" in result.results or "ParcelAggregation" in result.results

    # Check that we got regional damage values
    results_list = result.results.get("LocalDamage") or result.results.get("ParcelAggregation")
...
def test_matched_spaces_no_transformation_overhead(
    ...
    analyzer = LocalDamage(parcel_names=["test_atlas"])
...
def test_space_detection_from_metadata(lesion_nlin6_2mm):
    ...
    if expected_affine is not None:
        np.testing.assert_array_almost_equal(lesion_nlin6_2mm.affine, expected_affine, decimal=2)
...
def test_coordinate_space_creation():
    ...
    space = CoordinateSpace(identifier="MNI152NLin6Asym", resolution=2, reference_affine=affine)
