...
def lesion_mni152_1mm(tmp_path):
    ...
    return LesionData(
        lesion_img=img,
        metadata={"space": "MNI152NLin6Asym", "resolution": 1}
    )
...
class TestAnalysis2mm(BaseAnalysis):
    ...
    def _run_analysis(self, lesion_data):
        ...
        assert lesion_data.lesion_img.shape == (91, 109, 91), \
            f"Expected 2mm shape (91, 109, 91), got {lesion_data.lesion_img.shape}"
        return {"space": lesion_data.metadata.get("space"), "resolution": lesion_data.metadata.get("resolution")}
...
def test_error_when_lesion_missing_space_metadata():
    ...
    # This should raise error during LesionData creation
    with pytest.raises(ValueError, match="metadata must contain 'space' key"):
        LesionData(lesion_img=img)
