...
class TestGetLesionVoxelIndicesPerformance:
    ...
    def _setup_analysis(self, mock_connectome_info):
        ...
        analysis.logger = type('obj', (object,), {
...
class TestGetLesionVoxelIndicesVectorized:
    """Benchmark the new vectorized implementation."""
    ...
    def _setup_analysis(self, mock_connectome_info):
        ...
        # Inject mock connectome info
        analysis._mask_info = mock_connectome_info
        
        return analysis
    
    def _benchmark_vectorized(self, analysis, mask_data, label: str):
        """Benchmark the vectorized index retrieval."""
        # Warm-up
        _ = analysis._get_lesion_voxel_indices_vectorized(mask_data)
        ...
        for _ in range(n_trials):
            start = time.perf_counter()
            indices = analysis._get_lesion_voxel_indices_vectorized(mask_data)
            elapsed = time.perf_counter() - start
            times.append(elapsed)
    ...
    @pytest.mark.slow
    def test_vectorized_small_lesion(self, mock_connectome_info, create_lesion_mask):
        """Benchmark vectorized version with small lesion."""
        analysis = self._setup_analysis(mock_connectome_info)
        mask_data = create_lesion_mask(n_voxels=100)
        
        result = self._benchmark_vectorized(analysis, mask_data, "Vectorized - Small (100 voxels)")
        
        assert result["n_voxels"] == 100
        assert result["time_ms"] < 50  # Should be much faster than 112ms baseline
    
    @pytest.mark.slow
    def test_vectorized_large_lesion(self, mock_connectome_info, create_lesion_mask):
        """Benchmark vectorized version with large lesion."""
        analysis = self._setup_analysis(mock_connectome_info)
        mask_data = create_lesion_mask(n_voxels=10000)
        
        result = self._benchmark_vectorized(analysis, mask_data, "Vectorized - Large (10,000 voxels)")
...
@pytest.mark.slow
def test_full_benchmark_report(mock_connectome_info, create_lesion_mask):
    """Generate comprehensive benchmark report for documentation."""
    analysis = FunctionalNetworkMapping(
        connectome_name="mock_gsp1000",
        extraction_method="boes",
        log_level=0,
    ...
    print("\n" + "="*70)
    print("FUNCTIONAL NETWORK MAPPING - PERFORMANCE BENCHMARK")
    print("="*70)
    print(f"\nConnectome: {mock_connectome_info['mask_shape']} "
          f"({mock_connectome_info['n_voxels']:,} brain voxels)")
    print(f"Method: _get_lesion_voxel_indices() - Current O(N×M) Implementation")
    print(f"Platform: Python {pytest.__version__}")
    
    # Test different lesion sizes
    test_sizes = [100, 500, 1000, 5000, 10000]
    
    print("\n" + "-"*70)
    print("BENCHMARK RESULTS")
    print("-"*70)
    print(f"{'Lesion Size':<15} {'Time (ms)':<15} {'Time/Voxel (μs)':<20}")
    print("-"*70)
    ...
    for size in test_sizes:
        ...
        time_per_voxel_us = avg_time_ms * 1000 / size
        
        print(f"{size:,}".ljust(15) + 
              f"{avg_time_ms:.2f}".ljust(15) + 
              f"{time_per_voxel_us:.2f}")
    
    print("-"*70)
    print("\nOPTIMIZATION TARGET:")
    print("  Current: O(N×M) where N=lesion voxels, M=brain voxels")
    print("  Target: O(N) using vectorized lookup array")
    print("  Expected speedup: ~200x for coordinate matching")
    print("  Memory cost: ~3.6 MB (2mm), ~28.8 MB (1mm)")
    print("="*70)
