...
class TestAtlasAggregationOutputs:
    ...
    def test_atlas_aggregation_returns_roi_result(self, sample_mask_data, mock_atlas_file):
        ...
        # Should contain AtlasAggregationResult (may be empty if no overlap)
        roi_results = [r for r in results if isinstance(r, AtlasAggregationResult)]
        # All results should be AtlasAggregationResult type
        assert all(isinstance(r, AtlasAggregationResult) for r in results)

        if len(roi_results) > 0:
            # Check AtlasAggregationResult structure
    ...
    def test_atlas_aggregation_roi_result_has_metadata(self, sample_mask_data, mock_atlas_file):
        """AtlasAggregationResult from AtlasAggregation includes analysis metadata."""
        analysis = AtlasAggregation(
            atlas_names=["Schaefer2018_100Parcels7Networks"], aggregation="percent"
        )

        results = analysis._run_analysis(sample_mask_data)

        if len(results) > 0:
            roi_result = [r for r in results if isinstance(r, AtlasAggregationResult)][0]
