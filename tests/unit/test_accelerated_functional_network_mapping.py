"""Unit tests for AcceleratedFunctionalNetworkMapping analysis."""
...
def registered_atlas(tmp_path):
    nifti, labels_path, _labels = _write_tiny_atlas(tmp_path, "afnm_tiny")
    name = "afnm_tiny"
    register_parcellation_from_files(
        name=name,
...
def test_afnm_fractional_two_region_overlap(tmp_path, registered_atlas):
    ...
    results = out.results["AcceleratedFunctionalNetworkMapping"]
    lnm_key = next(k for k in results if k.endswith("desc-afnmap"))
    parc = results[lnm_key]
    assert parc.data["R1"] == pytest.approx(0.75)
    assert parc.data["R2"] == pytest.approx(0.75)


def test_afnm_binary_single_region(tmp_path, registered_atlas):
    ...
    out = analysis.run(subj)
    results = out.results["AcceleratedFunctionalNetworkMapping"]
    parc = next(v for k, v in results.items() if k.endswith("desc-afnmap"))
    assert parc.data["R1"] == pytest.approx(0.9)
    assert parc.data["R2"] == pytest.approx(0.3)


def test_afnm_voxel_count_weighting(tmp_path, registered_atlas):
...
def test_afnm_rejects_unknown_weighting(tmp_path, registered_atlas):
...
def test_afnm_requires_parcel_names(tmp_path):
    matrix_path = _write_c_matrix(tmp_path, ["R1", "R2"], np.eye(2))
    with pytest.raises(ValueError):
        AcceleratedFunctionalNetworkMapping(matrix_path=matrix_path, parcel_names=None)


def test_afnm_single_atlas_only(tmp_path, registered_atlas):
    matrix_path = _write_c_matrix(tmp_path, ["R1", "R2"], np.eye(2))
...
def test_afnm_mismatched_labels_error(tmp_path, registered_atlas):
...
def test_afnm_empty_overlap_returns_zero_map(tmp_path, registered_atlas):
    """Lesion that misses every region → m is zero; afnmap is zero; no crash."""
    C = np.array([[1.0, 0.5], [0.5, 1.0]])
    matrix_path = _write_c_matrix(tmp_path, ["R1", "R2"], C)
    mask = np.zeros(MASK_SHAPE, dtype=np.uint8)
...
def test_afnm_keep_intermediate_emits_weights(tmp_path, registered_atlas):
    """keep_intermediate=True → afnmweights ParcelData is emitted."""
    matrix_path = _write_c_matrix(tmp_path, ["R1", "R2"], np.eye(2))
    mask = np.zeros(MASK_SHAPE, dtype=np.uint8)
    mask[0, 0, 0] = 1  # R1
    subj = _make_subject(mask)
    ...
    out = analysis.run(subj)
    results = out.results["AcceleratedFunctionalNetworkMapping"]
    weights = next(v for k, v in results.items() if k.endswith("desc-afnmweights"))
...
def test_afnm_missing_matrix_file(tmp_path, registered_atlas):
    analysis = AcceleratedFunctionalNetworkMapping(
...
def test_afnm_bids_filename_resolves_to_method_afnm():
    """Guard against desc/source mapping drift: afnm result key → method-afnm."""
    from lacuna.core.keys import BidsFilename

    key = "atlas-schaefer400_source-AcceleratedFunctionalNetworkMapping_desc-afnmap"
    bids = BidsFilename.from_result_key(
        key, suffix="values", namespace="AcceleratedFunctionalNetworkMapping"
    )
    rendered = str(bids)
    assert "method-afnm_" in rendered
    assert "method-fnm_" not in rendered
    assert "desc-afnmap" in rendered
