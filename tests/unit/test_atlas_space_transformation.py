                ...
                with patch("lacuna.analysis.structural_network_mapping.compute_tdi_map"):
                    with patch(
                        ...
                        with patch("lacuna.spatial.transform.transform_image") as mock_transform:
                            ...
                            transformed_atlas_data = np.random.randint(
                                0, 101, size=(91, 109, 91), dtype=np.int16
                            )
                            transformed_atlas_img = nib.Nifti1Image(
                                transformed_atlas_data, np.eye(4)
                            )
                            ...
                            assert (
                                mock_transform.called
                            ), "Atlas should be transformed when spaces don't match"

                            # Verify transform was called with correct interpolation
                            call_kwargs = mock_transform.call_args[1]
                            assert (
                                call_kwargs.get("interpolation") == "nearest"
                    ...
                    with patch(
                        ...
                        with patch("lacuna.spatial.transform.transform_image") as mock_transform:
                            ...
                            mock_transform.side_effect = Exception(
                                "Should not transform when spaces match"
                            )
                ...
                with patch("lacuna.analysis.structural_network_mapping.compute_tdi_map"):
                    with patch(
                        ...
                        with patch("lacuna.spatial.transform.transform_image") as mock_transform:
                            ...
                            assert (
                                target_space.resolution == output_res
