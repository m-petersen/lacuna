...
class TestAtlasNamesFilter:
    ...
    def test_atlas_names_filters_correctly(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            analysis = LocalDamage(parcel_names=["atlas_B"])
            result = analysis.run(mask_data_obj)
            atlas_results = result.results["LocalDamage"]

            # Should only have atlas_B results
            # Keys use BIDS-style format: atlas-{atlas}_source-LocalDamage_desc-damagepct
            assert build_result_key("atlas_B", "LocalDamage", "damagepct") in atlas_results
            assert build_result_key("atlas_A", "LocalDamage", "damagepct") not in atlas_results
            assert build_result_key("atlas_C", "LocalDamage", "damagepct") not in atlas_results

            # Test 2: Process atlas_A and atlas_C
            analysis = LocalDamage(parcel_names=["atlas_A", "atlas_C"])
            result = analysis.run(mask_data_obj)
            atlas_results = result.results["LocalDamage"]

            # Should have atlas_A and atlas_C, but not atlas_B
            assert build_result_key("atlas_A", "LocalDamage", "damagepct") in atlas_results
            assert build_result_key("atlas_B", "LocalDamage", "damagepct") not in atlas_results
            assert build_result_key("atlas_C", "LocalDamage", "damagepct") in atlas_results
            ...
            analysis = LocalDamage(parcel_names=["atlas_A", "atlas_B", "atlas_C"])
            result = analysis.run(mask_data_obj)
            atlas_results = result.results["LocalDamage"]
    ...
    def test_atlas_names_warns_if_not_found(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            analysis = LocalDamage(parcel_names=["atlas_A", "atlas_B"])
    ...
    def test_atlas_names_works_with_atlas_aggregation(self):
        """Test that parcel_names works with ParcelAggregation (not just LocalDamage)."""
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir = Path(tmpdir)
