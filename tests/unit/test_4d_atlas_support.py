...
4. 4D atlas aggregation in FocalDamage
...
class Test4DParcelAggregation:
    ...
    def test_focal_damage_with_4d_atlas(self, tmp_path):
        """FocalDamage should work with 4D atlases."""
        # Create lesion
        ...
        try:
            ...
            # Run FocalDamage with 4D atlas
            analysis = FocalDamage(parcel_names=["Test4DAtlas_Aggregation"])

            result = analysis.run(lesion)

            # Check results - using BIDS-style keys
            damage_results = result.results["FocalDamage"]
            expected_key = "atlas-Test4DAtlas_Aggregation_source-FocalDamage_desc-damagepct"
    ...
    def test_4d_atlas_aggregation_per_volume(self, tmp_path):
        ...
        try:
            ...
            analysis = FocalDamage(parcel_names=["Test4D_VaryingOverlap"])

            result = analysis.run(lesion)
            damage_results = result.results["FocalDamage"]

            # Verify we got results for the atlas (BIDS-style naming)
            expected_key = build_result_key("Test4D_VaryingOverlap", "FocalDamage", "damagepct")
            assert expected_key in damage_results
            region_data = damage_results[expected_key].get_data()
            assert len(region_data) > 0
...
class TestMixed3DAnd4DAtlases:
    ...
    def test_focal_damage_with_mixed_atlases(self, tmp_path):
        """FocalDamage should handle mix of 3D and 4D atlases."""
        # Create lesion
        mask_data = np.zeros((20, 20, 20))
        mask_data[8:12, 8:12, 8:12] = 1
        lesion_affine = np.eye(4) * 2
        ...
        try:
            ...
            analysis = FocalDamage(parcel_names=["Mixed3D", "Mixed4D"])

            result = analysis.run(lesion)
            damage_results = result.results["FocalDamage"]

            # Should have results from both atlases (BIDS-style keys)
            key_3d = build_result_key("Mixed3D", "FocalDamage", "damagepct")
            key_4d = build_result_key("Mixed4D", "FocalDamage", "damagepct")
...
class TestFocalDamageOutputAPI:
    """Test correct usage of FocalDamage output API."""

    def test_focal_damage_returns_roi_result_list(self, tmp_path):
        """FocalDamage should return list of ParcelData objects, not dict."""
        # Create lesion
        mask_data = np.zeros((20, 20, 20))
        mask_data[8:12, 8:12, 8:12] = 1
        lesion_affine = np.eye(4) * 2
        ...
        try:
            ...
            analysis = FocalDamage(parcel_names=["TestOutputAPI"])
            ...
            assert isinstance(
                damage_results, dict
            ), "FocalDamage results should be a dict, not list"

            # Should have the atlas with BIDS-style key
            expected_key = build_result_key("TestOutputAPI", "FocalDamage", "damagepct")
            assert expected_key in damage_results
