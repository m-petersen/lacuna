...
class Test4DAtlasTransformation:
    ...
    def test_transform_4d_atlas_preserves_labels(self, tmp_path):
        ...
        for vol_idx in range(2):
            unique_vals = np.unique(result_data[..., vol_idx])
            assert all(val in [0, 1, 2] for val in unique_vals), \
...
class Test4DAtlasAggregation:
    ...
    def test_regional_damage_with_4d_atlas(self, tmp_path):
        ...
        # Create lesion
        lesion_data = np.zeros((20, 20, 20))
        lesion_data[8:12, 8:12, 8:12] = 1
        ...
        try:
            ...
            analysis = RegionalDamage(
                atlas_names=["Test4DAtlas_Aggregation"],
                threshold=0.0
            )
            ...
            # Note: Results structure depends on ROIResult implementation
    ...
    def test_4d_atlas_aggregation_per_volume(self, tmp_path):
        ...
        lesion_data = np.zeros((20, 20, 20))
        lesion_data[8:12, 8:12, 8:12] = 1  # Central lesion
        lesion_affine = np.eye(4) * 2
        lesion_affine[3, 3] = 1
        lesion_img = nib.Nifti1Image(lesion_data, lesion_affine)
        ...
        try:
            ...
            analysis = RegionalDamage(
                atlas_names=["Test4D_VaryingOverlap"],
                threshold=0.0
            )
...
class TestRegionalDamageOutputAPI:
    ...
    def test_regional_damage_returns_roi_result_list(self, tmp_path):
        """RegionalDamage should return list of ROIResult objects, not dict."""
        # Create lesion
        lesion_data = np.zeros((20, 20, 20))
        lesion_data[8:12, 8:12, 8:12] = 1
        lesion_affine = np.eye(4) * 2
        ...
        try:
            ...
            assert isinstance(damage_data, dict), \
                "ROIResult.get_data() should return a dict"
            
            # Damage data should be dict of region_name -> percentage
            assert all(isinstance(k, str) for k in damage_data.keys()), \
                "Region names should be strings"
            assert all(isinstance(v, (int, float)) for v in damage_data.values()), \
