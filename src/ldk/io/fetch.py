...
def get_data_dir() -> Path:
    ...
    Priority:
    1. LDK_DATA_DIR environment variable (explicit user choice)
    2. XDG_CACHE_HOME/ldk (XDG standard)
    3. ~/.cache/ldk (fallback)
    ...
    >>> print(data_dir)
    PosixPath('/home/user/.cache/ldk')

    >>> import os
    >>> os.environ['LDK_DATA_DIR'] = '/mnt/nvme/ldk_data'
    >>> data_dir = get_data_dir()
    >>> print(data_dir)
    PosixPath('/mnt/nvme/ldk_data')
    """
    if env_dir := os.getenv("LDK_DATA_DIR"):
        return Path(env_dir).expanduser().resolve()

    if xdg_cache := os.getenv("XDG_CACHE_HOME"):
        return Path(xdg_cache) / "ldk"

    return Path.home() / ".cache" / "ldk"
...
ATLAS_REGISTRY = pooch.create(
    ...
    base_url="https://github.com/lesion-decoding/ldk-data/raw/main/atlases/",
    registry={
...
def get_atlas(name_or_path: str) -> tuple[Path, Path]:
    ...
    >>> import os
    >>> os.environ['LDK_ATLAS_DIR'] = '/lab/atlases'
    ...
    if atlas_dir := os.getenv("LDK_ATLAS_DIR"):
    ...
    raise AtlasNotFoundError(
        ...
        f"  - Set LDK_ATLAS_DIR to search a custom directory\n"
...
def get_connectome_path(name_or_path: str) -> Path:
    ...
    >>> import os
    >>> os.environ['LDK_CONNECTOME_DIR'] = '/data/connectomes'
    ...
    if connectome_dir := os.getenv("LDK_CONNECTOME_DIR"):
        candidates = [
