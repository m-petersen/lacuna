...
- lacuna prepare: Precompute connectome-derived data products (e.g. AFNM matrices)
...
def build_parser(prog: str | None = None) -> ArgumentParser:
    ...
    parser = ArgumentParser(
        ...
        epilog=(
            ...
            "  prepare    Precompute connectome-derived data products (e.g. AFNM matrices)\n"
...
def _build_fetch_parser(subparsers) -> None:
    ...
    fetch_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "  gsp1000        - GSP1000 functional connectome (~220GB download + ~100GB HDF5,\n"
            "                   ~320GB peak on disk; requires Dataverse API key)\n"
            ...
            "For GSP1000, if <output-dir>/raw already contains the extracted\n"
            "sub-*/func/*finalmask files, the download is skipped and they are used\n"
            "directly (remove the raw dir to force a fresh download).\n\n"
            "Examples:\n"
...
def _add_prepare_afnm_args(p) -> None:
    ...
    g_io.add_argument(
        ...
        required=True,
        metavar="PATH",
        help="Path to the whole-brain functional connectome (voxelwise HDF5 file or directory).",
    )
    g_io.add_argument(
        "--output",
        "-o",
    ...
    g_parc = p.add_argument_group("Parcellation selection")
    g_parc.add_argument(
        "--parcel-atlases",
        nargs="+",
        type=str,
...
def _build_prepare_parser(subparsers) -> None:
    ...
    prepare_subparsers = prepare_parser.add_subparsers(
        dest="prepare_target",
        title="targets",
        description="Use 'lacuna prepare <target> --help' for more information.",
        metavar="<target>",
    )
