"""Simplified functional lesion network mapping (sfnm).

Implements the matrix-multiplication formulation of functional LNM from van
den Heuvel et al. (2026, *Nat Neurosci*): ``sfnmap = m @ C``, where ``m`` is
a lesion-to-parcel weight vector and ``C`` is a precomputed group-level
parcellated functional connectivity matrix (Fisher r-to-z values, produced
by ``lacuna parcellate --modality functional``).

Compared with voxelwise functional LNM this replaces a per-subject
...
class SimplifiedFunctionalNetworkMapping(BaseAnalysis):
    """Parcel-level functional LNM via matrix multiplication (``m @ C``).
    ...
    def __init__(
        ...
        if not parcel_names:
            raise ValueError(
                "parcel_names is required for SimplifiedFunctionalNetworkMapping "
        ...
        if len(parcel_names) != 1:
            raise ValueError(
                "SimplifiedFunctionalNetworkMapping currently supports exactly one "
    ...
    def _validate_inputs(self, mask_data: SubjectData) -> None:
        ...
        if mask_data.space is None:
            raise ValueError(
                "SimplifiedFunctionalNetworkMapping requires input with a known "
                "coordinate space."
            )
    ...
    def _run_analysis(self, mask_data: SubjectData) -> dict[str, "DataContainer"]:
        ...
        sfnmap_data = {lab: float(sfnmap[i]) for i, lab in enumerate(ordered_labels)}
        sfnmap_parc = ParcelData(
            name="sfnmap",
            data=sfnmap_data,
            region_labels=ordered_labels,
            parcel_names=[atlas_name],
            aggregation_method=f"sfnm_{self.lesion_weighting}",
            metadata=dict(meta_base, description="Simplified functional network map (m @ C)"),
        )
        results[
            build_result_key(
                atlas=atlas_name,
                source="SimplifiedFunctionalNetworkMapping",
                desc="sfnmap",
            )
        ] = sfnmap_parc
