...
def load_transform(name: str) -> Path:
    ...
    logger.debug(f"Querying TemplateFlow for forward transform: {from_space_normalized} → {to_space_normalized}")
                    ...
                    if file_size < 1024:  # Suspiciously small (< 1KB)
                        ...
                        if not path or not path.exists():
                            raise FileNotFoundError(
                                f"Failed to download valid transform file after retry"
                            )
                        file_size = path.stat().st_size
                    
                    logger.info(
                        f"✓ Transform loaded: {path.name} "
    ...
    logger.debug(f"Querying TemplateFlow for reverse transform: {to_space_normalized} → {from_space_normalized}")
