...
def _find_brain_mask(raw_dir: Path) -> Path:
    """Locate the brain mask used to extract the GSP1000 connectome.

    Prefers a real 3D mask shipped alongside the download. The GSP1000 release
    does not include one, so when none is found the canonical MNI152NLin6Asym
    2 mm brain mask is downloaded from OSF (and cached) via
    :func:`lacuna.assets.masks.load_brain_mask` — no TemplateFlow dependency.
    """
    ...
    # GSP1000 is in MNI152NLin6Asym 2 mm space.
    from ..assets.masks import load_brain_mask

    return load_brain_mask("MNI152NLin6Asym", 2.0)
